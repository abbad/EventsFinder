...
namespace Event_Finder.ViewModel
{
    public class FacebookViewModel
    {
        ...
        {
            return String.Format(@"SELECT eid, start_time, end_time, pic_big,pic_square, name, description, venue FROM event WHERE eid IN (SELECT eid FROM event_member WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = me()) OR uid = me() limit 3500) AND venue.latitude > ""{0}"" AND venue.latitude < ""{1}"" AND venue.longitude > ""{2}"" AND venue.longitude < ""{3}"" AND start_time >= ""{4}"" and start_time <= ""{5}""  ORDER BY start_time ASC LIMIT 50 ", 
                                       (latitude - offset).ToString(),
                                       (latitude + offset).ToString(),
                                       (longitude - offset).ToString(),
                                       (longitude + offset).ToString(),
